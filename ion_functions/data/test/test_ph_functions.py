...
REF = np.zeros((6, 16), dtype=np.int)       # reference measurements
LIGHT = np.zeros((6, 92), dtype=np.int)     # light measurements
BRAW = np.zeros(6, dtype=np.int)            # raw battery voltage counts
TRAW = np.zeros(6, dtype=np.int)            # raw instrument temperature in counts
for i in range(6):
    # parse the raw strings into subelements, such as the driver would
    # provide.
    s = raw_strings[i]
    BRAW[i] = int(s[455:459], 16)
    TRAW[i] = int(s[459:463], 16)
    strt = 19
    step = 4
    for j in range(16):
        REF[i, j] = int(s[strt:strt+step], 16)
        strt += step
...
class TestpHFunctionsUnit(BaseUnitTestCase):
    def test_ph_singles(self):
        ...
        for iRec in range(nRec):
            ...
            # record by record.
            bout[iRec] = BRAW[iRec] * 15. / 4096.
            tout[iRec] = phfunc.ph_thermistor(TRAW[iRec])
            pout[iRec] = phfunc.ph_calc_phwater(REF[iRec, :], LIGHT[iRec, :], tout[iRec],
                                                EA434, EB434, EA578, EB578)

        # test above output where records were processed one at a time
        self.assertTrue(np.allclose(bout, VBATT, rtol=1e-4, atol=0))
        self.assertTrue(np.allclose(tout, THERM, rtol=1e-4, atol=0))
        self.assertTrue(np.allclose(pout, PH, rtol=1e-4, atol=0))
    ...
    def test_ph_multiples(self):
        ...
        """
        bout = BRAW * 15. / 4096.
        tout = phfunc.ph_thermistor(TRAW)
        pout = phfunc.ph_calc_phwater(REF, LIGHT, tout, EA434, EB434, EA578, EB578)
