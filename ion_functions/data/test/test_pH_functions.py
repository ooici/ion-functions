...
class TestpHFunctionsUnit(BaseUnitTestCase):
    ...
    def test_pH_phwater(self):
        ...
        tfinal = np.array([25.8839, 25.8620, 25.8364,
                           25.8029, 25.7845, 25.7937])
        pH = np.array([8.0082, 8.0250, 8.0540,
                       8.0520, 8.0628, 8.0565])
        ...
        tout = np.zeros(6)
        for i in range(6):
            # parse the raw strings into subelements, as the driver would
            # provide.
            s = raw_strings[i]
            tstrt = int(s[16:19], 16)
            ...
            # compute the pH and final temperature in deg_C
            pHout[i], tout[i] = pHfunc.pH_phwater(ref, light, tstrt, tend)
        
        print pHout, tout
        
        self.assertTrue(np.allclose(pHout, pH, rtol=1e-4, atol=0))
        self.assertTrue(np.allclose(tout, tfinal, rtol=1e-4, atol=0))
