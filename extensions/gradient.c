 ...
 *
 * Arguments:
 * signed char *out           - An array of values that represent the evaluation of this QC. 
 *                              Only bad values and skipped values are set, all others are 
 *                              ignored. The implication here is that an array of ones,
 *                              should be passed in.
 * const double *dat          - An array of data values to apply QC against.
 * const double *x            - The axis for the data values, must be in increasing order.
 ...
 * double toldat              - The tolerance of d(dat)/dx to reach a good value.
 * const double skipped_value - Caller specified value to apply when qc evaluation is 
 *                              "skipped".
 */
